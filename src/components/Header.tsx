...
const Header: React.FC = () => {
    ...
    return (
        <>
            <header className="fixed top-0 w-full bg-white/90 backdrop-blur-xl shadow-lg z-30 transition-all duration-500 ease-out">
                <div className="container mx-auto px-3 sm:px-4 md:px-6">
                    <div className="flex justify-between items-center h-16 md:h-20">
                        {/* Logo Adaptativo */}
                        <Link to={languagePrefix} className="flex-shrink-0 flex items-center gap-2 sm:gap-3 min-w-0">
                            <img 
                                src="https://i.imgur.com/HKRbp5a.png" 
                                alt="Icono de Latitude Med Travel" 
                                className="h-10 sm:h-12 w-auto flex-shrink-0" 
                            />
                            <span className="hidden sm:block font-display text-lg md:text-xl lg:text-2xl tracking-wide uppercase whitespace-nowrap">
                        ...
                        <nav className="hidden md:flex items-center space-x-1 lg:space-x-2">
                            <a 
                                ...
                                className="text-gray-700 hover:bg-gray-200/70 font-medium px-3 lg:px-4 py-2 rounded-full transition-all duration-300 cursor-pointer hover:scale-105"
                            >
                                {t.header.nav.howItWorks}
                            </a>
                            <div className="relative group">
                                <button className="flex items-center gap-1.5 text-gray-700 hover:bg-gray-200/70 font-medium px-3 lg:px-4 py-2 rounded-full transition-all duration-300 hover:scale-105">
                                ...
                                <div className="absolute top-full left-1/2 -translate-x-1/2 mt-3 w-screen max-w-4xl p-4 opacity-0 scale-95 group-hover:scale-100 group-hover:opacity-100 invisible group-hover:visible transition-all duration-300 ease-out transform group-hover:translate-y-0 translate-y-2 z-50">
                                    <div className="bg-white rounded-2xl shadow-2xl border border-gray-100 grid grid-cols-2 gap-4 lg:gap-6 p-4 lg:p-6">
                                        ...
                                        <div>
                                            <div className="flex items-center gap-3 mb-4 pb-3 border-b border-gray-200">
                                                ...
                                                <h3 className="font-bold text-brand-blue-900 text-base lg:text-lg">{t.header.servicesMenu.facialProcedures}</h3>
                                            </div>
                                            <div className="space-y-1.5 lg:space-y-2">
                                                {servicios.filter(s => s.categoria === 'facial').map(servicio => (
                                                    <Link
                                                        ...
                                                        className="block px-3 py-2 rounded-lg hover:bg-gray-50 transition-all duration-200 text-sm text-gray-700 hover:text-brand-blue-900 hover:translate-x-1"
                                                    >
                                                        {servicio.nombre}
                                                    </Link>
                                                ))}
                                        ...
                                        <div>
                                            <div className="flex items-center gap-3 mb-4 pb-3 border-b border-gray-200">
                                                ...
                                                <h3 className="font-bold text-brand-blue-900 text-base lg:text-lg">{t.header.servicesMenu.bodyProcedures}</h3>
                                            </div>
                                            <div className="space-y-1.5 lg:space-y-2">
                        ...
                        <div className="flex items-center gap-1.5 sm:gap-2 md:gap-3 flex-shrink-0">
                            {/* Consultation Button - Visible desde md */}
                            <a 
                                href="#contacto" 
                                onClick={(e) => handleSectionLink('#contacto', e)}
                                className="hidden md:inline-block bg-brand-yellow-400 text-brand-blue-900 font-semibold py-2 px-4 lg:px-5 rounded-full text-xs lg:text-sm hover:bg-brand-yellow-500 transition-all duration-300 shadow-md hover:shadow-lg cursor-pointer hover:scale-105 whitespace-nowrap"
                            >
                            ...
                            {/* Services Icon - Solo en móvil/tablet */}
                            ...
                            >
                                ...
                                {item && (
                                    <span className={`absolute -top-1 -right-1 bg-brand-yellow-400 text-brand-blue-900 text-xs font-bold rounded-full h-5 w-5 flex items-center justify-center transition-all duration-500 shadow-md ${cartAnimation ? 'scale-125 animate-pulse' : 'scale-100'}`}>
                                        1
                                    </span>
                                )}
                            </button>
                            
                            {/* Hamburger Menu - Solo en móvil/tablet */}
                            <button 
                                onClick={handleMenuToggle} 
                                className="md:hidden p-2 rounded-full hover:bg-gray-100 transition-all duration-300 hover:scale-110"
                                aria-label="Menu"
                            >
                                <svg id="menu-open-icon" className="w-6 h-6 sm:w-7 sm:h-7 text-brand-blue-900" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h16m-7 6h7"></path></svg>
                            </button>
                        </div>
                    </div>
                </div>
